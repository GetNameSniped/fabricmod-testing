package net.canbot.canmod.util.player;
...
public class Rotations {
    ...
    public static double getPitchFromTo(Entity entity, BlockPos pos) {
        double y = pos.getY();

        double diffX = pos.getX() - entity.getX();
        double diffY = y - (entity.getY() + entity.getEyeHeight(entity.getPose()));
        double diffZ = pos.getZ() - entity.getZ();

        double diffXZ = Math.sqrt(diffX * diffX + diffZ * diffZ);

        return entity.getPitch() + MathHelper.wrapDegrees((float) -Math.toDegrees(Math.atan2(diffY, diffXZ)) - entity.getPitch());
    };

    public static double getYawFromTo(Entity entity, BlockPos pos) {

        return entity.getYaw() + MathHelper.wrapDegrees((float) Math.toDegrees(Math.atan2(pos.getZ() + 0.5 - entity.getZ(), pos.getX() + 0.5 - entity.getX())) - 90f - entity.getYaw());
    }

    public static ArrayList<Double> doStuff(BlockPos entity, BlockPos pos) {
